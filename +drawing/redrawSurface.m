...
if true
    ...
    motionFcn = @() camlight(c, 0,40);
    motionFcn();
    set (gcf, 'WindowButtonMotionFcn',@(src,obj) motionFcn());
else
    h = rotate3d;
    set( h, 'ActionPreCallback', @gui.perspectiveChange_Callback); %called right before user changes perspective
    set( h, 'ActionPostCallback', @gui.perspectiveChange_Callback); %called after user changes perspective
    set(h,'Enable','on');
    %view( v.vprefs.az,  v.vprefs.el);
    if ~isempty(v.vprefs.camLight)
        delete(v.vprefs.camLight); % - delete previous lights!
    end
    v.vprefs.camLight = camlight( 0, 40); %2015 consider changing to 0,10 6767
end
