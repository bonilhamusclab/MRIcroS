...
%function [Hdr, Vol] = readNifti(filename)
%file can be unzipped nifti (nii.gz) or nifti (.nii)
%  http://undocumentedmatlab.com/blog/savezip-utility
%To do:
%  endian: rare, currently detected and reported
...
if fileUtils.isExt('.hdr',filename) && fileUtils.isExt('.img',Hdr.fname) %analyze style .hdr and .img pairs
...
Vol = spm_read_vols_Sub(Hdr, data);
%end nii()
